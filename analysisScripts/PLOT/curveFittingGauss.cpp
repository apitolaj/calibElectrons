...
//normalization function (kept for reference / future use -- currently NOT called,
//since we are fitting raw, unnormalized event counts)
...
void curveFittingGauss_SOURCE_PLACEHOLDER(const char *inputtedROOTFile1)
{
...
//	Normalize(h1_noEnv);   // intentionally left off -- fitting raw counts
	...
	TF1 *fitFunc = new TF1("fitFunc", sixGaussianSingleFit, xmin, xmax, 5);   // 6 ? 8

	fitFunc->SetParNames("n1", "mu1", "sig1", "n4", "mu4");

	double totalCounts = h1_noEnv->Integral();

	fitFunc->SetParameters(0.2 * totalCounts, 450.0, 20.0, 0.5 * totalCounts, 960.0);
	
	fitFunc->SetParLimits(0, 0.0, 2.0 * totalCounts);
	fitFunc->SetParLimits(1, 400.0, 500.0);
	fitFunc->SetParLimits(2, 10.0, 60.0);	
	fitFunc->SetParLimits(3, 0.0, 5.0 * totalCounts);
	fitFunc->SetParLimits(4, 940.0, 980.0);

	//resulting fitted six gaussian function
	TFitResultPtr fitResult = h1_noEnv->Fit(fitFunc, "RSI+I");
	...
	gPad->Update();  // make sure the title primitive actually exists
	...
	//get the envelope tree from the root file and create a histogram of the electron energy spectrum;
	//this is not drawn on the canvas but used later for fitting the total electron energy spectrum.
	//NOTE: also left UNNORMALIZED so it stays on the same count scale as h1_noEnv.
...
//	Normalize(h1_env);   // intentionally left off -- fitting raw counts
	...
	h2_data->SetTitle("Bi207 Calibration Source Energy Spectrum (all electrons);Energy [keV];Counts");
	...
	{
        // Gaussian contribution: counts / keV
        ...
        // Convert histogram counts/bin into counts/keV
	...
	leg2->AddEntry(fCombined, "Gaussian Fit", "l");
	...
	c->cd(0);
	c->SaveAs("fittedGauss_SOURCE_PLACEHOLDER.png");
}
