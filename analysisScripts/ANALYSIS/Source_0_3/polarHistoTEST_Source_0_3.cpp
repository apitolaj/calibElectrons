...
void polarHistoTEST_Source_0_3(const char* filename)
{
    gStyle->SetOptStat(0);
    ...
    bool okNoEnv = DrawTreeRow(
        f,
        "noEnvelope",
        "Source 0_3 (No env., all energies)",
        "noenv",
        c->GetPad(3),
        c->GetPad(4),
        noenvLeft,
    ...
    c->Update();
    c->SaveAs("polarHisto_Source_0_3.png");
    ...
    if (okEnv && okNoEnv)
    {
        ...
        cRatio->Update();
        cRatio->SaveAs("polarHisto_Source_0_3_ratios.png");
    }
    else
    {
        std::cerr
