...
bool isEdgeTPP(MiEvent *Event)
{
	...
	double TPPy = TPPposVector->getY();
	double TPPz = TPPposVector->getZ();
	
	vector<MiCDCaloHit>* caloHitVector = Event->getPTD()->getpart(0)->getcalohitv();
	
   	vector<string> primaryElectronOMs;
	
	for(MiCDCaloHit& caloHit : *caloHitVector)
	{
		MiGID* ID = caloHit.getGID();
		
		if(ID->getwall()=="calo")
		{
			primaryElectronOMs.push_back(ID->getmodule());
		}
	}
	
	if (primaryElectronOMs.empty()) return false;
	
	for(int i =0; i < primaryElectronOMs.size() - 1; i++)
	{
		if(primaryElectronOMs[i] != primaryElectronOMs[i+1]) return false;
	}
	
	
	auto [OMy, OMz] = sourcePositions.at(primaryElectronOMs[0]);
	...
	(
		...
		(((OMz+edgeCutRange < TPPz) && (TPPz < OMz+edgeDistFromCenter)) || ((OMz-edgeCutRange > TPPz) && (TPPz > OMz-edgeDistFromCenter)))
