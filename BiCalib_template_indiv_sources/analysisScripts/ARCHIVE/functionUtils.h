...
MiVector3D* calculatePosTPPVector(vector<MiVertex>* vertexVector);

void populateChain(TChain &ROOTChain, const char *dirPath);

double calculateDistTPP(vector<MiVertex>* vertexVector, MiVector3D *calibSourceVertexPos);

double calculateDistOM(vector<MiVertex>* vertexVector, MiVector3D *calibSourceVertexPos);

bool isEdgeTPP(vector<MiVertex>* vertexVector);
...
inline void makeTrees(TFile* outFile, treeData& trees) 
{
	...
	t->Branch("DistOM"  	    ,   &trees.DistOM    	,  "DistOM/D");
        ...
        t->Branch("SDInitialEnergy" ,   &trees.SDInitialEnergy  ,  "SDInitialEnergy/D");
